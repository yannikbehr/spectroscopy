...
class MiniDoasPluginTestCase(unittest.TestCase):
    ...
    def test_missing_entries(self):
        ...
        e = d.read(os.path.join(self.data_dir,
                                'SR_20160530_missing_entries.csv'),
    ...
    def read_single_station(self, d, station_info):
        ...
        bearing = (np.ones(rb.d_var.shape[0])
                   * np.rad2deg(station_info['bearing']))
        ...
        # Now read in preferred flux values for
        # assumed height downloaded from FITS
        ...
        # Now read in preferred flux values for calculated height downloaded
        # from FITS
        ...
        indices = []
        for i, dt in enumerate(dates):
            idx = np.argmin(np.abs(f1.datetime[:].astype('datetime64[s]')
                                   - dt))
            indices.append(idx)
