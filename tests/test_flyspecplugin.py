...
class FlySpecPluginTestCase(unittest.TestCase):
    ...
    def test_add(self):
        ...
        rb, cb = d1.read(os.path.join(self.data_dir, '2016_06_11_0830_TOFP04.txt'),
        ...
        rb, cb = d2.read(os.path.join(self.data_dir, '2016_06_11_0900_TOFP04.txt'),
                         ftype='FLYSPEC', timeshift=12.0)
        r = d2.new(rb)
        cb.rawdata = r
        d2.new(cb)
    ...
    def test_read(self):
        ...
        rb, cb = d.read(os.path.join(self.data_dir,'2012_02_29_1340_CHILE.txt'),
                        ftype='FLYSPEC')
        r = d.new(rb)
        cb.rawdata = r
        c = d.new(cb)
    ...
    def test_not_enough_data(self):
        with self.assertRaises(FlySpecPluginException):
            d1 = Dataset(tempfile.mktemp(), 'w')
            rb, cb = d1.read(os.path.join(self.data_dir, '2015_05_03_1630_TOFP04.txt'),
                             ftype='FLYSPEC', timeshift=12.0)
