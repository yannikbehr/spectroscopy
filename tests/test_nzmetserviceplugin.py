...
class NZMetservicePluginTestCase(unittest.TestCase):
    ...
    def test_empty_model(self):
        ...
        with self.assertRaises(NZMetservicePluginException):
            filename = 'gns_wind_model_data_ecmwf_20141228_0630.txt'
            d1.read(os.path.join(self.data_dir, filename),
                    ftype='NZMETSERVICE', preferred_model='ecmwf')
        d1.read(os.path.join(self.data_dir,
                             'gns_wind_model_data_ecmwf_20141228_0630.txt'),
                ftype='NZMETSERVICE')

    def test_date_bug(self):
        d = Dataset(tempfile.mktemp(), 'w')
        fin = os.path.join(self.data_dir,
                           'gns_wind_model_data_ukmo_20160127_0630.txt')
        d.read(fin, ftype='NZMETSERVICE')
        gf = d.elements['GasFlow'][0]
        res = get_wind_speed(gf, 174.735, -36.890, 1000,
                             '2016-01-27T06:00:00+13:00')
        (lon, lat, hght, time, vx, vx_error,
         vy, vy_error, vz, vz_error, dist) = res
